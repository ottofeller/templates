...
describe('NextJS template', () => {
  ...
  describe('has jest and an example test', () => {
    test('setup by default', () => {
      ...
      expect(snapshot['package.json'].scripts.test).toEqual('npx projen test')
      expect(snapshot['package.json'].scripts).toHaveProperty('test-unit:watch')
      expect(snapshot['.projen/tasks.json'].tasks).toHaveProperty('test')
      expect(snapshot['.projen/tasks.json'].tasks.test.steps[0].exec).toEqual('jest --no-cache --all')
      expect(snapshot['.projen/tasks.json'].tasks).toHaveProperty('test-unit:watch')
