...
test('two NextJS config files are present', () => {
  const project = new TestNextJsTypeScriptProject()
  const snapshot = synthSnapshot(project)
...
test('prettier and eslint configs are written to package.json', () => {
  const project = new TestNextJsTypeScriptProject()
  const snapshot = synthSnapshot(project)
...
describe('default test workflow', () => {
  ...
  test('does not exists if opted out', () => {
